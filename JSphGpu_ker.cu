...
__device__ void VanAlbadaLimiter(float beta,float &a){

	if (beta>0){
	a= (beta*beta+beta)/(1+beta*beta);
	} else {
		a=0;
	}
...
template<TpKernel tker,TpFtMode ftmode,bool lamsps,TpDensity tdensity,bool shift,bool symm>
  ...
  ,TpShifting shiftmode,float4 &shiftposfsp1,const float3 *gradrho, tmatrix3d *lrenor,const float3 *deltales,double *eigen,float3 &gradeigen)
{
  for(int p2=pini;p2<pfin;p2++){
    const float4 pscellp2=poscell[p2];
    float drx=pscellp1.x-pscellp2.x + CTE.poscellsize*(CEL_GetfX(pscellp1.w)-CEL_GetfX(pscellp2.w));
    ...
    if(rr2<=CTE.kernelsize2 && rr2>=ALMOSTZERO){
      ...
      if (tdensity==DDT_Green && deltap1!=FLT_MAX){
    	const float3 gradrhop1 = gradrho[p1];
        const float3 gradrhop2 = gradrho[p2];
              // LIMITER FORMULATION
        float limit1, limit2;
        const float gradlim1 =  (gradrhop1.x*drx+gradrhop1.y*dry+gradrhop1.z*drz)/(velrhop2.w-velrhop1.w);
        const float gradlim2 = -(gradrhop2.x*drx+gradrhop2.y*dry+gradrhop2.z*drz)/(velrhop1.w-velrhop2.w);
        if (abs(velrhop1.w-velrhop2.w)>10e-6){

          VanAlbadaLimiter(gradlim1,limit1);
          VanAlbadaLimiter(gradlim2,limit2);

        } else {
           limit1=1.0;
           limit2=1.0;
        }
            //  printf("%21.16f %21.16f %21.16f %21.16f \n",limit1,limit2,gradlim1,gradlim2);
          const float rhopp1_r = velrhop1.w + 0.5*limit1*((-drx)*gradrhop1.x+(-dry)*gradrhop1.y+(-drz)*gradrhop1.z);
          const float rhopp2_r = velrhop2.w - 0.5*limit2*((-drx)*gradrhop2.x+(-dry)*gradrhop2.y+(-drz)*gradrhop2.z);

      //        const float psi=(rhopp2_r-rhopp1_r);
      //        const float delta=2.0*cbar*CTE.h*psi*((-drx)*frx+(-dry)*fry+(-drz)*frz)*(massp2/velrhop2.w)/rr2;
          const float psi_ab=sqrt(drx*drx+dry*dry+drz*drz)*(1.0/CTE.kernelh)*(2.0*velrhop1.w)*(rhopp2_r-rhopp1_r)/(rhopp1_r+rhopp2_r);
          const float dwdq= ((-drx)*frx+(-dry)*fry+(-drz)*frz)*CTE.kernelh/sqrt(drx*drx+dry*dry+drz*drz);
          const float delta=2*cbar*psi_ab*(massp2/velrhop2.w)*dwdq/sqrt(drx*drx+dry*dry+drz*drz);
      	  deltap1=(boundp2 && CTE.tboundary==BC_DBC? FLT_MAX: deltap1+delta);
      }
      if (tdensity==DDT_DeltaLes && deltap1!=FLT_MAX){
          	 const float3 gradrhop1 = gradrho[p1];
             const float3 gradrhop2 = gradrho[p2];
             const float gradrho_dot = ((gradrhop1.x==FLT_MAX||gradrhop1.y==FLT_MAX||gradrhop1.z==FLT_MAX)? 0 :((gradrhop2.x==FLT_MAX||gradrhop2.y==FLT_MAX||gradrhop2.z==FLT_MAX)? 0: (gradrhop1.x+gradrhop2.x)*(-drx)+(gradrhop1.y+gradrhop2.y)*(-dry)+(gradrhop1.z+gradrhop2.z)*(-drz)));
             const float psi = (velrhop2.w-velrhop1.w)-0.5*gradrho_dot;
             const float dot31=((-drx)*frx+(-dry)*fry+(-drz)*frz);
             const float delta_dynamic=2*(deltales[p1].x*deltales[p2].x)/(deltales[p1].x+deltales[p2].x);
             const float delta=delta_dynamic*psi*(massp2/velrhop2.w)*dot31/(rr2);//+CTE.eta2);
             deltap1=(boundp2 && CTE.tboundary==BC_DBC? FLT_MAX: deltap1+delta);
       }
	  ...
	  if (normcalc) {
		  const tmatrix3d lrenorp1 = lrenor[p1];
		  float4 velrhop2 = velrhop[p2];
		  double rhopp2 = double(velrhop[p2].w);
		  const double Vol2 = double(massp2 / rhopp2);
		  const double eigenp1 = eigen[p1];
		  const double eigenp2 = eigen[p2];
		  if (idp[p1] == 2044) { printf("\nfluidbox lrenor \n"); }
		  if (idp[p1] == 2044) { printf("%21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f \n", lrenorp1.a11, lrenorp1.a12, lrenorp1.a13, lrenorp1.a21, lrenorp1.a22, lrenorp1.a23, lrenorp1.a31, lrenorp1.a32, lrenorp1.a33); }
		  if (idp[p1] == 2044) { printf("\nfluidbox eigenp1 \n"); }
		  if (idp[p1] == 2044) { printf("%21.16f \n", eigen[p1]); }
		  if (idp[p1] == 2044) { printf("\nfluidbox eigenp2 \n"); }
		  if (idp[p1] == 2044) { printf("%21.16f \n", eigen[p2]); }
		  //const double determ = cumath::Determinant3x3(lrenorp1);
		  //const tmatrix3d inv_lrenor = cumath::InverseMatrix3x3(lrenorp1, determ);
		  double3 rhs1;
		  float3 gradeigen;
		  double3 x1;
		  x1.x = 0.; x1.y = 0.; x1.z = 0.;
		  
		  rhs1.x = (-eigenp1 + eigenp2)*Vol2*frx;
		  rhs1.y = (-eigenp1 + eigenp2)*Vol2*fry;
		  rhs1.z = (-eigenp1 + eigenp2)*Vol2*frz;
		  KerLUSolve(lrenorp1, rhs1, x1);
		  gradeigen.x += x1.x; 
		  gradeigen.y += x1.y;
		  gradeigen.z += x1.z;

		  if (idp[p1] == 2044) { printf("\ngradeigen fluidbox :\n"); }
		  if (idp[p1] == 2044) { printf("%21.16f %21.16f %21.16f \n", gradeigen.x, gradeigen.y, gradeigen.z); }

		  //if (idp[p1] == 132) { printf("check3"); }
		  //if (idp[p1] == 132) { printf("%21.16f %21.16f %21.16f \n", gradeigen.x, gradeigen.y, gradeigen.z); }
		  	  
		  // need to calculate the inverse of lrenor & pass the eigen values in the forcesfluidbox
		  //gradeigenx += (-eigenp1 + eigenp2)*Vol2*inv_lrenor*frx;

		  //Lcorr.a31 += Vol2*(-drz)*frx; Lcorr.a32 += Vol2*(-drz)*fry; Lcorr.a33 += Vol2*(-drz)*frz;
		  //rhs1.x += dvelx*Vol2*frx;      rhs1.y += dvelx*Vol2*fry;      rhs1.z += dvelx*Vol2*frz;

	  }

	 // add a flag for calc norm before the if statement load the data and t
      //-Shifting correction.
      ...
      if(compute){
        ...
        if(!lamsps){//-Artificial viscosity.
          ...
          else{ //-LES model - this overwrites the Artificial Viscosity when DDT_DeltaLes is used.
        		const float K=(float) CTE.KDeltaLes;
        		const float alpha=visco;
        		const float alpha_ij=alpha+2*(deltales[p1].y*deltales[p2].y)/(deltales[p1].y+deltales[p2].y);
        		const float amubar=dot_rr2;  //amubar=CTE.h*dot/(rr2+CTE.eta2);
        		const float q_les=(deltales[p1].z+deltales[p2].z)*(USE_FLOATING? ftmassp2: massp2)/velrhop2.w;
        		const float bho=(CTE.rhopzero/velrhop1.w)*(USE_FLOATING? ftmassp2: massp2)*(1.0/velrhop2.w);
        		const double pi_visc=(-K*alpha_ij*amubar*bho);
        		if (CTE.tboundary!=BC_DBC || !boundp2){
        		acep1.x-=(-K*alpha_ij*amubar*bho)*frx; acep1.y-=(-K*alpha_ij*amubar*bho)*fry; acep1.z-=(-K*alpha_ij*amubar*bho)*frz;
        		//if(abs((-K*alpha_ij*amubar*bho)*frx)>1000){
        		//printf("%d %d %21.16f %21.16f %21.16f %21.16f  %21.16f %21.16f %21.16f %21.16f\n",p1,p2,abs((-8.0*alpha_ij*amubar*bho)*frx),acep1.x,(pi_visc)*frx,pi_visc,amubar,K*alpha_ij,bho,frx);
        		}//}
        	}
        }
        else{//-Morris viscosity model
            const float temp=2.f*visco/((rr2+CTE.eta2)*velrhop2.w);
        ...
        //   //-Velocity gradients for SPS turbulence model.
...
template<TpKernel tker,TpFtMode ftmode,bool lamsps,TpDensity tdensity,bool shift,bool symm>
  ...
  ,TpShifting shiftmode,float4 *shiftposfs,float3 *gradrho, tmatrix3d *lrenor,float3 *deltales, double *eigen)
{
  const unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    const unsigned p1=p+pinit;      //-Number of particle.
    float visc=0,arp1=0,deltap1=0;
    float3 acep1=make_float3(0,0,0);//initialize the gradeigen here
	float3 gradeigen = make_float3(0, 0, 0);
    ...
    for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
      ...
      if(pfin){
                          KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,false> (false,p1,pini,pfin,viscof,ftomassp,tauff,dengradcorr,poscell,velrhop,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,visc,deltap1,shiftmode,shiftposfsp1,gradrho,lrenor,deltales,eigen,gradeigen);
        if(symm && rsymp1)KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,true > (false,p1,pini,pfin,viscof,ftomassp,tauff,dengradcorr,poscell,velrhop,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,visc,deltap1,shiftmode,shiftposfsp1,gradrho,lrenor,deltales,eigen, gradeigen); //<vs_syymmetry>
      }
    }
    ...
    for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
      unsigned pini,pfin=0;  cunsearch::ParticleRange(c2,c3,ini1,fin1,begincell,pini,pfin);
      if(pfin){
                        KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,false> (true ,p1,pini,pfin,viscob,ftomassp,tauff,NULL,poscell,velrhop,code,idp,CTE.massb,ftp1,pscellp1,velrhop1,pressp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,visc,deltap1,shiftmode,shiftposfsp1,gradrho,lrenor,deltales,eigen, gradeigen);
      if(symm && rsymp1)KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,true > (true ,p1,pini,pfin,viscob,ftomassp,tauff,NULL,poscell,velrhop,code,idp,CTE.massb,ftp1,pscellp1,velrhop1,pressp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,visc,deltap1,shiftmode,shiftposfsp1,gradrho,lrenor,deltales,eigen, gradeigen);
      }
    }
    ...
    if(shift||arp1||acep1.x||acep1.y||acep1.z||visc){
      if(tdensity!=DDT_None){
		...
		//printf("%i %21.16f %21.16f %21.16f %21.16f %21.16f \n",idp[p1],deltap1,arp1,pscellp1.x+CTE.poscellsize*(CEL_GetfX(pscellp1.w)),pscellp1.y+CTE.poscellsize*(CEL_GetfY(pscellp1.w)),pscellp1.z+CTE.poscellsize*(CEL_GetfZ(pscellp1.w)));
	...
	float gradeigennorm = sqrt(gradeigen.x*gradeigen.x + gradeigen.y*gradeigen.y + gradeigen.z*gradeigen.z);
	gradeigen.x = gradeigen.x / gradeigennorm; gradeigen.y = gradeigen.y / gradeigennorm; gradeigen.z = gradeigen.z / gradeigennorm;
	if (idp[p1] == 2044) { printf("\nforces fluid gradeigen :\n"); }
	if (idp[p1] == 2044) { printf("%21.16f %21.16f %21.16f \n", gradeigen.x, gradeigen.y, gradeigen.z); }
  }
}
  ...
  void Interaction_ForcesT_KerInfo(StKerInfo *kerinfo)
{
  ...
  {
    typedef void (*fun_ptr)(unsigned,unsigned,float,float,int,int4,int3,const int2*,unsigned,const unsigned*,const float*,const float2*,float2*,const float3*,const float4*,const float4*,const typecode*,const unsigned*,float*,float*,float3*,float*,TpShifting,float4*,float3*, tmatrix3d*,float3*,double*);
...
{ //add a print statement about what i addded (eign / lcorr) ie t.lammin[t.idp[132]] - and check that is the same with the printed value from before
  //-Collects kernel information.
#ifndef DISABLE_BSMODES
  if(t.kerinfo){
  ...
  if(t.fluidnum){
    ...
    if(t.symmetry) //<vs_syymmetry_ini>
      ...
      ,t.viscdt,t.ar,t.ace,t.delta,t.shiftmode,t.shiftposfs,t.gradrhog,t.lrenorg,t.deltales,t.eigen);
...
template<TpKernel tker,TpFtMode ftmode,bool lamsps> void Interaction_Forces_gt2(const StInterParmsg &t){
  ...
  if(t.shiftmode || t.tdensity!=DDT_DeltaLes)throw "Shifting and extra DDT are disabled for FastCompilation...";
  Interaction_ForcesGpuT<tker,ftmode,lamsps,DDT_DeltaLes,false> (t);
#else
  if(t.shiftmode){               const bool shift=true;
    ...
    if(t.tdensity==DDT_Green)	Interaction_ForcesGpuT<tker,ftmode,lamsps,DDT_Green	  ,shift> (t);
    if(t.tdensity==DDT_DeltaLes)Interaction_ForcesGpuT<tker,ftmode,lamsps,DDT_DeltaLes,shift> (t);
  }
...
__device__ void KerLUSolve(tmatrix3d A,double3 b1,double3 b2,double3 b3,double3 b4,float3 &x1,float3 &x2,float3 &x3,float3 &x4)
{
  double U11=A.a11, U12=A.a12, U13=A.a13;
  double U21=A.a21, U22=A.a22, U23=A.a23;
  double U31=A.a31, U32=A.a32, U33=A.a33;
  double L21,L31,L32;
  double temp1,temp2,temp3,temp4, temp5,temp6,temp7;

  //row1
  temp1=U11; temp2=U12; temp3=U13; temp4=b1.x;temp5=b2.x;temp6=b3.x;temp7=b4.x;
  if(abs(U21)>abs(U11)&&abs(U21)>abs(U31)){
    U11=U21;   U12=U22;   U13=U23;
    U21=temp1; U22=temp2; U23=temp3;
    b1.x=b1.y;   b1.y=temp4;
    b2.x=b2.y;   b2.y=temp5;
    b3.x=b3.y;   b3.y=temp6;
    b4.x=b4.y;   b4.y=temp7;

  }
  else if(abs(U31)>abs(U11)&&abs(U31)>abs(U21)){
    U11=U31;   U12=U32;   U13=U33;
    U31=temp1; U32=temp2; U33=temp3;
    b1.x=b1.z;   b1.z=temp4;
    b2.x=b2.z;   b2.z=temp5;
    b3.x=b3.z;   b3.z=temp6;
    b4.x=b4.z;   b4.z=temp7;
  }
  L21=U21/U11;
  U21=0;
  U22=U22-L21*U12;
  ...
  //row2
  temp1=U22; temp2=U23; temp3=L21; temp4=b1.y; temp5=b2.y; temp6=b3.y; temp7=b4.y;
  if(abs(U32)>abs(U22)) {
    U22=U32;   U23=U33;
    U32=temp1; U33=temp2;
    L21=L31;   L31=temp3;
    b1.y=b1.z;   b1.z=temp4;
    b2.y=b2.z;   b2.z=temp5;
    b3.y=b3.z;   b3.z=temp6;
    b4.y=b4.z;   b4.z=temp7;
  }
  L32=U32/U22;
  U32=0.0;
  U33=U33-L32*U23;
  ...
  //solver
  double y1=b1.x;
  double y2=b1.y-L21*y1;
  double y3=b1.z-L32*y2-L31*y1;

  x1.z=float(y3/U33);
  x1.y=float((y2-U23*x1.z)/U22);
  x1.x=float((y1-U12*x1.y-U13*x1.z)/U11);


  y1=b2.x;
  y2=b2.y-L21*y1;
  y3=b2.z-L32*y2-L31*y1;

  x2.z=float(y3/U33);
  x2.y=float((y2-U23*x2.z)/U22);
  x2.x=float((y1-U12*x2.y-U13*x2.z)/U11);

  y1=b3.x;
  y2=b3.y-L21*y1;
  y3=b3.z-L32*y2-L31*y1;

  x3.z=float(y3/U33);
  x3.y=float((y2-U23*x3.z)/U22);
  x3.x=float((y1-U12*x3.y-U13*x3.z)/U11);

  y1=b4.x;
  y2=b4.y-L21*y1;
  y3=b4.z-L32*y2-L31*y1;

   x4.z=float(y3/U33);
   x4.y=float((y2-U23*x4.z)/U22);
   x4.x=float((y1-U12*x4.y-U13*x4.z)/U11);
  if(abs(U11*U22*U33)<1e-8) {
    // printf("found bad det! \n");
    x1.x=FLT_MAX; x1.y=FLT_MAX; x1.z=FLT_MAX;
  }
}

//------------------------------------------------------------------------------
// Kernel to solve a 2x2 linear system using LU with row pivoting
//------------------------------------------------------------------------------
__device__ void KerLUSolve(tmatrix3d A,double3 b1,double3 b2,double3 b3,float2 &x1,float2 &x2,float2 &x3)
{
  double U11=A.a11, U12=A.a13;
  double U21=A.a31, U22=A.a33;
  double L21=0.0;
  double temp1,temp2,temp3,temp4,temp5;

  //row1
  temp1=U11; temp2=U12; temp3=b1.x; temp4=b2.x; temp5=b3.x;
  if (abs(U21)>abs(U11)){
    U11=U21;   U12=U22;
    U21=temp1; U22=temp2;
    b1.x=b1.z;   b1.z=temp3;
    b2.x=b2.z;	 b2.z=temp4;
    b3.x=b3.z;	 b3.z=temp5;
  ...
  //solver
  double y1=b1.x;
  double y2=b1.z-L21*y1;

  x1.y=float(y2/U22);
  x1.x=float((y1-U12*x1.y)/U11);

  y1=b2.x;
  y2=b2.z-L21*y1;

  x2.y=float(y2/U22);
  x2.x=float((y1-U12*x2.y)/U11);

  y1=b3.x;
  y2=b3.z-L21*y1;

  x3.y=float(y2/U22);
  x3.x=float((y1-U12*x3.y)/U11);
  if(abs(U11*U22)<1e-8){
    //printf("found bad det! \n");
    x1.x=FLT_MAX; x1.y=FLT_MAX;
  }
}


__device__ void KerLUSolve(tmatrix3d A, double3 b1, double3 &x1)
{
	double U11 = A.a11, U12 = A.a12, U13 = A.a13;
	double U21 = A.a21, U22 = A.a22, U23 = A.a23;
	double U31 = A.a31, U32 = A.a32, U33 = A.a33;
	...
	double temp1, temp2, temp3, temp4;
		...
		//row1
		temp1 = U11; temp2 = U12; temp3 = U13; temp4 = b1.x;
	if (abs(U21)>abs(U11) && abs(U21)>abs(U31)) {
		U11 = U21;   U12 = U22;   U13 = U23;
		U21 = temp1; U22 = temp2; U23 = temp3;
		b1.x = b1.y;   b1.y = temp4;
	...
	//row2
	temp1 = U22; temp2 = U23; temp3 = L21; temp4 = b1.y;
	if (abs(U32)>abs(U22)) {
		U22 = U32;   U23 = U33;
		U32 = temp1; U33 = temp2;
		L21 = L31;   L31 = temp3;
	...
	x1.z = (y3 / U33);
	x1.y = ((y2 - U23*x1.z) / U22);
	x1.x = ((y1 - U12*x1.y - U13*x1.z) / U11);
...
template<TpKernel tker,TpFtMode ftmode,bool symm>
  __device__ void KerComputeGradRhoBox(bool boundp2,unsigned p1
  ,const unsigned &pini,const unsigned &pfin,float visco
  ,const float *ftomassp,const float2 *tauff,const float3 *dengradcorr
  ,const float4 *poscell,const float4 *velrhop,const typecode *code,const unsigned *idp
  ,float massp2,bool ftp1
  ,const float4 &pscellp1,const float4 &velrhop1,float pressp1
  ,tmatrix3d &Lcorr,double3 &rhs1,double3 &rhs2,double3 &rhs3,double3 &rhs4)
{
  for(int p2=pini;p2<pfin;p2++){
    const float4 pscellp2=poscell[p2];
    float drx=pscellp1.x-pscellp2.x + CTE.poscellsize*(CEL_GetfX(pscellp1.w)-CEL_GetfX(pscellp2.w));
...
//        if(ftp2 && shift && shiftmode==SHIFT_NoBound)shiftposfsp1.x=FLT_MAX; //-Cancels shifting with floating bodies. | Con floatings anula shifting.
      ...
      if(symm)velrhop2.y=-velrhop2.y; //<vs_syymmetry>
       double rhopp2= double(velrhop[p2].w);
       double rhopp1= double(velrhop1.w);
      //rhopp2 =0.111111111111*(pscellp1.x+drx)+0.111111111111*(pscellp1.z+drz);
      //rhopp1 =0.111111111111*pscellp1.x+0.111111111111*pscellp1.z;
      //-Velocity derivative (Momentum equation).
      const double Vol2=double(massp2/rhopp2);
      const double drhop=double(-rhopp1+rhopp2);
      const double dvelx=double(-velrhop1.x+velrhop2.x);
      const double dvely=double(-velrhop1.y+velrhop2.y);
      const double dvelz=double(-velrhop1.z+velrhop2.z);

      Lcorr.a11+=Vol2*(-drx)*frx; Lcorr.a12+=Vol2*(-drx)*fry; Lcorr.a13+=Vol2*(-drx)*frz;
      Lcorr.a21+=Vol2*(-dry)*frx; Lcorr.a22+=Vol2*(-dry)*fry; Lcorr.a23+=Vol2*(-dry)*frz;
      Lcorr.a31+=Vol2*(-drz)*frx; Lcorr.a32+=Vol2*(-drz)*fry; Lcorr.a33+=Vol2*(-drz)*frz;
      rhs1.x+=dvelx*Vol2*frx;      rhs1.y+=dvelx*Vol2*fry;      rhs1.z+=dvelx*Vol2*frz;
      rhs2.x+=dvely*Vol2*frx;      rhs2.y+=dvely*Vol2*fry;      rhs2.z+=dvely*Vol2*frz;
      rhs3.x+=dvelz*Vol2*frx;      rhs3.y+=dvelz*Vol2*fry;      rhs3.z+=dvelz*Vol2*frz;
      rhs4.x+=drhop*Vol2*frx;      rhs4.y+=drhop*Vol2*fry;      rhs4.z+=drhop*Vol2*frz;
...
//------------------------------------------------------------------------------
template<TpKernel tker,TpFtMode ftmode,bool symm>
  __global__ void KerComputeGradRho(unsigned n,unsigned pinit,float viscob,float viscof
  ,int scelldiv,int4 nc,int3 cellzero,const int2 *begincell,unsigned cellfluid,const unsigned *dcell
  ,const float *ftomassp,const float2 *tauff,float2 *gradvelff,const float3 *dengradcorr
  ,const float4 *poscell,const float4 *velrhop,const typecode *code,const unsigned *idp
  ,float *viscdt,float *ar,float3 *ace,float *delta
  ,TpShifting shiftmode,float4 *shiftposfs,float3 *gradrho, tmatrix3d *lrenor,float3 *gradvelx,float3 *gradvely,float3 *gradvelz,bool simulate2d, double *eigen)
{
  const unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    const unsigned p1=p+pinit;      //-Number of particle.
...
//      if(ftp1 && shift)shiftposfsp1.x=FLT_MAX; //-Shifting is not calculated for floating bodies. | Para floatings no se calcula shifting.
    ...
    tmatrix3d Lcorr;//=TMatrix3d(0.0);
    Lcorr.a11=0.0;Lcorr.a12=0.0;Lcorr.a13=0.0;
    Lcorr.a21=0.0;Lcorr.a22=0.0;Lcorr.a23=0.0;
    Lcorr.a31=0.0;Lcorr.a32=0.0;Lcorr.a33=0.0;
    double3 rhs1=make_double3(0.0,0.0,0.0);
    double3 rhs2=make_double3(0.0,0.0,0.0);
    double3 rhs3=make_double3(0.0,0.0,0.0);
    double3 rhs4=make_double3(0.0,0.0,0.0);
    ...
    for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
      unsigned pini,pfin=0;  cunsearch::ParticleRange(c2,c3,ini1,fin1,begincell,pini,pfin);
      if(pfin){
    	  	  	  	  	  KerComputeGradRhoBox<tker,ftmode,false> (false,p1,pini,pfin,viscof,ftomassp,tauff,dengradcorr,poscell,velrhop,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,Lcorr,rhs1,rhs2,rhs3,rhs4);
        if(symm && rsymp1)KerComputeGradRhoBox<tker,ftmode,true > (false,p1,pini,pfin,viscof,ftomassp,tauff,dengradcorr,poscell,velrhop,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,Lcorr,rhs1,rhs2,rhs3,rhs4); //<vs_syymmetry>
      }
    }
    //-Interaction with boundaries.
    ini3-=cellfluid; fin3-=cellfluid;
    for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
      unsigned pini,pfin=0;  cunsearch::ParticleRange(c2,c3,ini1,fin1,begincell,pini,pfin);
      if(pfin){
    	  	  	  	  	KerComputeGradRhoBox<tker,ftmode,false> (true ,p1,pini,pfin,viscob,ftomassp,tauff,NULL,poscell,velrhop,code,idp,CTE.massb,ftp1,pscellp1,velrhop1,pressp1,Lcorr,rhs1,rhs2,rhs3,rhs4);
      if(symm && rsymp1)KerComputeGradRhoBox<tker,ftmode,true > (true ,p1,pini,pfin,viscob,ftomassp,tauff,NULL,poscell,velrhop,code,idp,CTE.massb,ftp1,pscellp1,velrhop1,pressp1,Lcorr,rhs1,rhs2,rhs3,rhs4);
      }
    }
    
    

    float3 x1=make_float3(0.f,0.f,0.f);
    float3 x2=make_float3(0.f,0.f,0.f);
    float3 x3=make_float3(0.f,0.f,0.f);
    float3 x4=make_float3(0.f,0.f,0.f);

    if(simulate2d){
    			float2 x1=make_float2(0.f,0.f);
    			float2 x2=make_float2(0.f,0.f);
    			float2 x4=make_float2(0.f,0.f);
    			KerLUSolve(Lcorr,rhs1,rhs3,rhs4,x1,x2,x4);
    			if(isnan(x4.x)){
    			x1=make_float2(0.f,0.f);
    			x2=make_float2(0.f,0.f);
    			x4=make_float2(0.f,0.f);
    			}

    			gradrho[p1]=make_float3(x4.x,0.f,x4.y);
    			gradvelx[p1]=make_float3(x1.x,0.f,x1.y);
    			gradvelz[p1]=make_float3(x2.x,0.f,x2.y);
    			gradvely[p1]=make_float3(0.0,0.0,0.0);
    			//printf("%i %21.16f %21.16f %21.16f %21.16f %21.16f \n",idp[p1],x1.x,x1.y,pscellp1.x+CTE.poscellsize*(CEL_GetfX(pscellp1.w)),pscellp1.y+CTE.poscellsize*(CEL_GetfY(pscellp1.w)),pscellp1.z+CTE.poscellsize*(CEL_GetfZ(pscellp1.w)));
    			...
    			float3 x3=make_float3(0.f,0.f,0.f);
    			float3 x4=make_float3(0.f,0.f,0.f);
    			KerLUSolve(Lcorr,rhs1,rhs2,rhs3,rhs4,x1,x2,x3,x4);
    			
				double alpha, beta, gamma, invp, invq, invphi, lam1, lam2, lam3, lammin;

				alpha = Lcorr.a11 + Lcorr.a22 + Lcorr.a33;
				beta = Lcorr.a12*Lcorr.a12 + Lcorr.a13*Lcorr.a13 + Lcorr.a23*Lcorr.a23 - Lcorr.a11*Lcorr.a22 - Lcorr.a22*Lcorr.a33 - Lcorr.a33*Lcorr.a11;
				gamma = Lcorr.a11*Lcorr.a22*Lcorr.a33 + 2.0*Lcorr.a12*Lcorr.a23*Lcorr.a13 - Lcorr.a11*Lcorr.a23*Lcorr.a23 - Lcorr.a12*Lcorr.a12*Lcorr.a33 - Lcorr.a13*Lcorr.a13*Lcorr.a22;
				invp = -(3.0*beta + alpha*alpha) / 3.0;
				invq = -(gamma + 2.0*alpha*alpha*alpha / 27.0 + alpha*beta / 3.0);
				invphi = acos(-(invq) / (2.0*sqrt((abs(invp) / 3.0))*sqrt((abs(invp) / 3.0))*sqrt((abs(invp)) / 3.0)));

				lam1 = alpha / 3.0 + 2.0*sqrt(abs(invp) / 3.0)*cos(invphi / 3); //1st eigen value
				lam2 = alpha / 3.0 - 2.0*sqrt(abs(invp) / 3.0)*cos((invphi - M_PI) / 3.0); // 2nd eigen value
				lam3 = alpha / 3.0 - 2.0*sqrt(abs(invp) / 3.0)*cos((invphi + M_PI) / 3.0); // 3rd eigen value

				//finds min eigenvalue

				if (lam1 > lam2) lammin = lam2;
				else lammin = lam1;
				if (lam1 > lam3) lammin = lam3;
				else lammin = lam1;
				if (lam2 > lam3) lammin = lam3;
				else lammin = lam2;

    			gradrho[p1]=make_float3(x4.x,x4.y,x4.z);
    			gradvelx[p1]=make_float3(x1.x,x1.y,x1.z);
    			gradvely[p1]=make_float3(x2.x,x2.y,x2.z);
    			gradvelz[p1]=make_float3(x3.x,x3.y,x3.z);
				eigen[p1] = lammin;

				//tmatrix3d lrenor;//=TMatrix3d(0.0);
				//lrenor.a11 = Lcorr.a11; lrenor.a12 = Lcorr.a12; lrenor.a13 = Lcorr.a13;
				//lrenor.a21 = Lcorr.a12; lrenor.a22 = Lcorr.a22; lrenor.a23 = Lcorr.a23;
				//lrenor.a31 = Lcorr.a31; lrenor.a32 = Lcorr.a32; lrenor.a33 = Lcorr.a33;
				lrenor[p1] = Lcorr;
    			
    			//save min eigval as a float & Lcorr matrix  as tmatrix3d

    			//printf("%i %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f \n",idp[p1],x1.x,x1.y,x1.z,posdp1.x,posdp1.y,posdp1.z);
    			//printf("Printing idpp1");
    			//printf("%i \n",idp[p1]);
				if (idp[p1] == 2044) { printf("kergradrho Lcorr \n"); }
    			if(idp[p1]== 2044){printf("%21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f \n",Lcorr.a11,Lcorr.a12,Lcorr.a13,Lcorr.a21,Lcorr.a22,Lcorr.a23,Lcorr.a31,Lcorr.a32,Lcorr.a33);}
				if (idp[p1] == 2044) { printf("kergradrho lrenor \n"); }
				if (idp[p1] == 2044) { printf("%21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f %21.16f \n", lrenor[p1].a11, lrenor[p1].a12, lrenor[p1].a13, lrenor[p1].a21, lrenor[p1].a22, lrenor[p1].a23, lrenor[p1].a31, lrenor[p1].a32, lrenor[p1].a33); }

				//if (idp[p1] == 132) { printf("lam1,lam2,lam3 and lammin \n"); }
				//if(idp[p1]==132){printf("%21.16f %21.16f %21.16f \n",lam1,lam2,lam3);}
				//if (idp[p1] == 132) { printf("%21.16f \n", lammin); }
				if (idp[p1] == 2044) { printf("kergradrho eigenp1 \n"); }
				if (idp[p1] == 2044) { printf("%21.16f \n", eigen[p1]); }

				//if(idp[p1]==132){printf("%i \n",idp[p1]);}
				//printf("%i \n",idp[p1]);
...
#ifndef DISABLE_BSMODES
template<TpKernel tker,TpFtMode ftmode,bool symm>
  void Compute_GradRhoT_KerInfo(StKerInfo *kerinfo)
{
 #if CUDART_VERSION >= 6050
  {
    typedef void (*fun_ptr)(unsigned,unsigned,float,float,int,int4,int3,const int2*,unsigned,const unsigned*,const float*,const float2*,float2*,const float3*,const float4*,const float4*,const typecode*,const unsigned*,float*,float*,float3*,float*,TpShifting,float4*,float3*,tmatrix3d*,float3*,float3*,float3*,bool,double*);
    fun_ptr ptr=&KerComputeGradRho<tker,ftmode,symm>;
    int qblocksize=0,mingridsize=0;
    cudaOccupancyMaxPotentialBlockSize(&mingridsize,&qblocksize,(void*)ptr,0,0);
    struct cudaFuncAttributes attr;
    cudaFuncGetAttributes(&attr,(void*)ptr);
...
template<TpKernel tker,TpFtMode ftmode>
  void Compute_GradRhoGpuT(const StInterParmsg &t) //add here the eigenvalue array 
{
  //-Collects kernel information.
#ifndef DISABLE_BSMODES
  if(t.kerinfo){
	  Compute_GradRhoT_KerInfo<tker,ftmode,false>(t.kerinfo);
    return;
  }
#endif
  const StDivDataGpu &dvd=t.divdatag;
  ...
  //-Interaction Fluid-Fluid & Fluid-Bound.
  if(t.fluidnum){
    dim3 sgridf=GetSimpleGridSize(t.fluidnum+t.vnpb,t.bsfluid+t.bsbound);
    if(t.symmetry) //<vs_syymmetry_ini>
      KerComputeGradRho<tker,ftmode,true> <<<sgridf,t.bsfluid+t.bsbound,0,t.stm>>>
      (t.fluidnum+t.vnpb,t.boundini,t.viscob,t.viscof,dvd.scelldiv,dvd.nc,dvd.cellzero,dvd.beginendcell,dvd.cellfluid,t.dcell
      ,t.ftomassp,(const float2*)t.tau,(float2*)t.gradvel,t.dengradcorr,t.poscell,t.velrhop,t.code,t.idp
      ,t.viscdt,t.ar,t.ace,t.delta,t.shiftmode,t.shiftposfs,t.gradrhog,t.lrenorg,t.gradvelx,t.gradvely,t.gradvelz,t.simulate2d,t.eigen);
    else //<vs_syymmetry_end>
    	KerComputeGradRho<tker,ftmode,false> <<<sgridf,t.bsfluid+t.bsbound,0,t.stm>>>
      (t.fluidnum+t.vnpb,t.boundini,t.viscob,t.viscof,dvd.scelldiv,dvd.nc,dvd.cellzero,dvd.beginendcell,dvd.cellfluid,t.dcell
      ,t.ftomassp,(const float2*)t.tau,(float2*)t.gradvel,t.dengradcorr,t.poscell,t.velrhop,t.code,t.idp
      ,t.viscdt,t.ar,t.ace,t.delta,t.shiftmode,t.shiftposfs,t.gradrhog,t.lrenorg,t.gradvelx,t.gradvely,t.gradvelz,t.simulate2d,t.eigen);
      //KerInteractionForcesFluid<tker,ftmode,lamsps,tdensity,shift,false> <<<sgridf,t.bsfluKerComputeGradRhoid,0,t.stm>>> (t.fluidnum,t.fluidini,t.scelldiv,t.nc,t.cellfluid,t.viscob,t.viscof,t.begincell,Int3(t.cellmin),t.dcell,t.ftomassp,(const float2*)t.tau,(float2*)t.gradvel,t.poscell,t.velrhop,t.code,t.idp,t.viscdt,t.ar,t.ace,t.delta,t.shiftmode,t.shiftposfs);
  }
}
template<TpKernel tker> void Compute_GradRhog_gt0(const StInterParmsg &t){
#ifdef FAST_COMPILATION
  if(t.ftmode!=FTMODE_None)throw "Extra FtMode options are disabled for FastCompilation...";
  Compute_GradRhoGpuT<tker,FTMODE_None> (t);
#else
  if(t.ftmode==FTMODE_None)    Compute_GradRhoGpuT<tker,FTMODE_None> (t);
  else if(t.ftmode==FTMODE_Sph)Compute_GradRhoGpuT<tker,FTMODE_Sph>  (t);
  else if(t.ftmode==FTMODE_Ext)Compute_GradRhoGpuT<tker,FTMODE_Ext>  (t);
#endif
}

void Compute_GradRhog(const StInterParmsg &t){
#ifdef FAST_COMPILATION
  if(t.tkernel!=KERNEL_Wendland)throw "Extra kernels are disabled for FastCompilation...";
  Compute_GradRhog_gt0<KERNEL_Wendland> (t);
#else
  if(t.tkernel==KERNEL_Wendland)     Compute_GradRhog_gt0<KERNEL_Wendland> (t);
 #ifndef DISABLE_KERNELS_EXTRA
  else if(t.tkernel==KERNEL_Cubic)   Compute_GradRhog_gt0<KERNEL_Cubic   > (t);
 #endif
#endif
//  cudaDeviceSynchronize();
//  abort();
...
__global__ void KerComputeDeltaLes(bool simulate2d,unsigned n,unsigned pini,float smag,float blin
  ,const float4 *velrhop,const float3 *gradvelx,const float3 *gradvely,const float3 *gradvelz,float3 *deltales)
{
    unsigned K=10;
    if(simulate2d) K=8;
  ...
  if(p<n){
    const unsigned p1=p+pini;
    const float grad_xx=gradvelx[p1].x;		        const float grad_xy=gradvelx[p1].y+gradvely[p1].x;
    const float grad_xz=gradvelx[p1].z+gradvelz[p1].x;  const float grad_yy=gradvely[p1].y;
    const float grad_yz=gradvely[p1].z+gradvelz[p1].y;  const float grad_zz=gradvelz[p1].z;
    const float pow1=grad_xx*grad_xx + grad_yy*grad_yy + grad_zz*grad_zz;
    const float prr= grad_xy*grad_xy + grad_xz*grad_xz + grad_yz*grad_yz + pow1+pow1;
    const float nu_delta=(pow(1.5*CTE.kernelh,2.0))*sqrt(prr);
    const float nu_visco=(pow(0.12*CTE.kernelh,2.0))*sqrt(prr);
    deltales[p1].x=(nu_delta>10e-9?nu_delta:10e-9);
    deltales[p1].y=(nu_visco>10e-9?nu_visco:10e-9);
    deltales[p1].z=2.0*0.044*pow(CTE.kernelh,2.0f)*prr;
    //if(p1==23788)printf("%i %21.16f %21.16f %21.16f %21.16f %21.16f\n",p1,grad_xx,grad_xz,grad_zz,prr,deltales[p1]);
  }
}
...
void ComputeDeltaLes(unsigned np,unsigned npb,float smag,float blin
  ,const float4 *velrhop,const float3 *gradvelx,const float3 *gradvely,const float3 *gradvelz,float3 *deltales,bool simulate2d,cudaStream_t stm)
{
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetSimpleGridSize(npf,SPHBSIZE);
    KerComputeDeltaLes <<<sgridf,SPHBSIZE,0,stm>>> (simulate2d,npf,npb,smag,blin,velrhop,gradvelx,gradvely,gradvelz,deltales);
  }
}
//##############################################################################
