...
typedef struct StrInterParmsg{
  ...
  float3 *gradrhog;
  tmatrix3d *lrenorg;
  float3 *gradvelx; float3 *gradvely; float3 *gradvelz;
  float3 *deltales;
  ...
  StrInterParmsg(
    ...
    ,float3 *gradrhog_
	, tmatrix3d *lrenorg_
    ,float3 *gradvelx_, float3 *gradvely_, float3 *gradvelz_
    ,float3 *deltales_
    ,cudaStream_t stm_
    ,StKerInfo *kerinfo_, double *eigen_)
  {
    //-Configuration options.
    ...
    gradrhog=gradrhog_;
	lrenorg = lrenorg_;
    gradvelx=gradvelx_;gradvely=gradvely_;gradvelz=gradvelz_;
    deltales=deltales_;
    //-Other values and objects.
    stm=stm_;
    kerinfo=kerinfo_;
...
void ComputeDeltaLes(unsigned np,unsigned npb,float smag,float blin
  ,const float4 *velrhop,const float3 *gradvelx,const float3 *gradvely,const float3 *gradvelz,float3 *deltales,bool simulate2d,cudaStream_t stm=NULL);
