...
void JSphGpuSingle::ConfigDomain(){
  ...
  //for (unsigned p = 0; p<Np; p++) Eigen[p] = 0;
...
void JSphGpuSingle::RunCellDivide(bool updateperiodic){
  ...
  TmgStart(Timers,TMG_NlSortData);
  {
	...
	swap(Eigeng, eigeng); ArraysGpu->Free(eigeng);
  }
  if(TStep==STEP_Verlet){
	...
	// Eigenvalue: sort array EigenM1g
	CellDivSingle->SortDataArrays(EigenM1g, eigeng); // Overloaded function!
	swap(EigenM1g, eigeng); ArraysGpu->Free(eigeng);
  }
	...
	CellDivSingle->SortDataArrays(EigenPreg, eigeng);
	swap(EigenPreg, eigeng); ArraysGpu->Free(eigeng);
  }
...
void JSphGpuSingle::Interaction_Forces(TpInterStep interstep){
  ...
  if(TDensity==DDT_Green || TDensity==DDT_DeltaLes){cusph::Compute_GradRhog(parms);
  cusph::ComputeDeltaLes(Np,Npb,SpsSmag,SpsBlin,Velrhopg,GradVelxg,GradVelyg,GradVelzg,DeltaLes,Simulate2D);
  //if(interstep==2) DgSaveVtkParticlesGpu("BB_ComputeStep.vtk",Nstep,0,Np,Posxyg,Poszg,Codeg,Idpg,Velrhopg,GradRhog,GradVelxg,DeltaLes,GradVelzg,Arg);
  }
//abort();
 ...
 // DgSaveVtkParticlesGpu("BB_ComputeStepA.vtk",interstep,0,Np,Posxyg,Poszg,Codeg,Idpg,Velrhopg,GradRhog,GradVelzg,DeltaLes,Aceg,Arg);
//abort();
