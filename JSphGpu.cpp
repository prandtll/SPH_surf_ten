...
JSphGpu::JSphGpu(bool withmpi):JSph(false,false,withmpi),DivAxis(MGDIV_None){
  ClassName="JSphGpu";
  Idp=NULL; Code=NULL; Dcell=NULL; Posxy=NULL; Posz=NULL; Velrhop=NULL; Eigen = NULL;
  AuxPos=NULL; AuxVel=NULL; AuxRhop=NULL; AuxEigen = NULL;
  CellDiv=NULL;
...
void JSphGpu::InitVars(){
  ...
  FreeCpuMemoryParticles();
  FreeCpuMemoryFixed();
  Idpg=NULL; Codeg=NULL; Dcellg=NULL; Posxyg=NULL; Poszg=NULL; PosCellg=NULL; Velrhopg=NULL; Eigeng = NULL;
  BoundNormalg=NULL; MotionVelg=NULL; //-mDBC
  VelrhopM1g=NULL; EigenM1g = NULL;									  //-Verlet
  PosxyPreg=NULL; PoszPreg=NULL; VelrhopPreg=NULL; EigenPreg = NULL; //-Symplectic
  SpsTaug=NULL; SpsGradvelg=NULL;									//-Laminar+SPS. 
  ViscDtg=NULL; 
  Arg=NULL; Aceg=NULL; Deltag=NULL; GradEigeng = NULL;
  GradRhog=NULL; Lrenorg = NULL;
  GradVelxg=NULL; GradVelyg=NULL; GradVelzg=NULL;
  DeltaLes=NULL;
...
void JSphGpu::AllocCpuMemoryParticles(unsigned np){
  ...
  if(np>0){
    try{
	  ...
	  Eigen = new double[np]; MemCpuParticles += sizeof(double)*np;
	  AuxEigen = new double[np]; MemCpuParticles += sizeof(double)*np;
    }
    catch(const std::bad_alloc){
...
void JSphGpu::AllocGpuMemoryParticles(unsigned np,float over){
  ...
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_8B, 2); // Eigeng //reminder : i need to add gradeigen here
  if(TStep==STEP_Verlet){
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_16B,1); //-velrhopm1
	ArraysGpu->AddArrayCount(JArraysGpu::SIZE_8B, 1); // Eigenvalue: EigenM1
  }
	...
	ArraysGpu->AddArrayCount(JArraysGpu::SIZE_8B, 1); // Eigenvalue: EigenPre
  }
  ...
  if(TDensity==DDT_Green || TDensity==DDT_DeltaLes){
	  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B,1); //-gradrho
	  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_72B, 2); //-Lrenor
	  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B, 1); //-GradEigen
	  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B,4); //-gradvelx,y,z
  }
  if(UseNormals){
...
void JSphGpu::ReserveBasicArraysGpu(){
  ...
  PosCellg=ArraysGpu->ReserveFloat4();
  Velrhopg=ArraysGpu->ReserveFloat4();
  Eigeng = ArraysGpu->ReserveDouble();
  if (TStep == STEP_Verlet) {
	  VelrhopM1g = ArraysGpu->ReserveFloat4();
	  EigenM1g = ArraysGpu->ReserveDouble();
  }
...
void JSphGpu::ParticlesDataUp(unsigned n,const tfloat3 *boundnormal){
  ...
  cudaMemcpy(Eigeng ,Eigen ,sizeof(double)*n ,cudaMemcpyHostToDevice);
...
void JSphGpu::InitRunGpu(){
  ...
  cudaMemcpy(EigenM1g, Eigeng, sizeof(double)*Np, cudaMemcpyDeviceToDevice);  }
...
void JSphGpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  if(GradRhog)cudaMemset(GradRhog,0,sizeof(tfloat3)*np);   			 //GradRhog[]=(0,0,0)
  if (Lrenorg)cudaMemset(Lrenorg, 0, sizeof(tmatrix3d)*np);				//Lrenor[[]]=((0,0,0),(0,0,0),(0,0,0))
  if (GradEigeng)cudaMemset(GradEigeng, 0, sizeof(tfloat3)*np);   			 //GradEigeng[]=(0,0,0) 
  if(GradVelxg)cudaMemset(GradVelxg,0,sizeof(tfloat3)*np);   			 //GradRhog[]=(0,0,0)
  if(GradVelyg)cudaMemset(GradVelyg,0,sizeof(tfloat3)*np);   			 //GradRhog[]=(0,0,0)
  if(GradVelzg)cudaMemset(GradVelzg,0,sizeof(tfloat3)*np);   			 //GradRhog[]=(0,0,0)
  if(DeltaLes)cudaMemset(DeltaLes,0,sizeof(tfloat3)*np);   			 //GradRhog[]=(0,0,0)
...
void JSphGpu::PreInteraction_Forces(){
  ...
  if(TDensity==DDT_Green || TDensity==DDT_DeltaLes){
	  GradRhog=ArraysGpu->ReserveFloat3();
	  Lrenorg=ArraysGpu->ReserveMatrix3d();
	  GradEigeng = ArraysGpu->ReserveFloat3();
	  GradVelxg=ArraysGpu->ReserveFloat3();
	  GradVelyg=ArraysGpu->ReserveFloat3();
	  GradVelzg=ArraysGpu->ReserveFloat3();
	  DeltaLes=ArraysGpu->ReserveFloat3();
  }
  //-Initialise arrays.
...
void JSphGpu::PosInteraction_Forces(){
  ...
  ArraysGpu->Free(GradRhog);     GradRhog=NULL;
  ArraysGpu->Free(Lrenorg);     Lrenorg = NULL;
  ArraysGpu->Free(GradEigeng);     GradEigeng = NULL;
  ArraysGpu->Free(GradVelxg);    GradVelxg=NULL;
  ArraysGpu->Free(GradVelyg);    GradVelyg=NULL;
  ArraysGpu->Free(GradVelzg);    GradVelzg=NULL;
  ArraysGpu->Free(DeltaLes);     DeltaLes=NULL;
}
...
/// Graba fichero VTK con datos de las particulas (debug).
//==============================================================================
void JSphGpu::DgSaveVtkParticlesGpu(std::string filename,int numfile,unsigned pini,unsigned pfin,const double2 *posxyg,const double *poszg,const typecode *codeg,const unsigned *idpg,const float4 *velrhopg,const float3 *gradrhog,const float3 *gradvelxg,const float3 *gradvelyg,const float3 *gradvelzg,const float *arg){
  //-Allocates memory.
  const unsigned n=pfin-pini;
  tfloat3 *pos=new tfloat3[n];
  {
  ...
  tfloat3 *vel=NULL;
  float *rhop=NULL;
  tfloat3 *gradrho=NULL;
  tfloat3 *gradvelx=NULL;
  tfloat3 *gradvely=NULL;
  tfloat3 *gradvelz=NULL;
  byte *type=NULL;
   if(codeg){
     type=new byte[n];
     typecode *aux=new typecode[n];
  ...
  if(velrhopg){
        ...
        tfloat4 *aux=new tfloat4[n];
        cudaMemcpy(aux,velrhopg+pini,sizeof(float4)*n,cudaMemcpyDeviceToHost);
        //cudaMemcpy(rhop,arg+pini,sizeof(float)*n,cudaMemcpyDeviceToHost);
        for(unsigned p=0;p<n;p++){ vel[p]=TFloat3(aux[p].x,aux[p].y,aux[p].z); rhop[p]=aux[p].w; }
  ...
  if(gradrhog)gradrho=new tfloat3[n];
  if(gradvelxg)gradvelx=new tfloat3[n];
  if(gradvelyg)gradvely=new tfloat3[n];
  if(gradvelzg)gradvelz=new tfloat3[n];

  //-Copies data from GPU.
  if(idpg)cudaMemcpy(idp,idpg+pini,sizeof(unsigned)*n,cudaMemcpyDeviceToHost);
  if(gradrhog)cudaMemcpy(gradrho,gradrhog+pini,sizeof(float3)*n,cudaMemcpyDeviceToHost);
  if(gradvelxg)cudaMemcpy(gradvelx,gradvelxg+pini,sizeof(float3)*n,cudaMemcpyDeviceToHost);
  if(gradvelyg)cudaMemcpy(gradvely,gradvelyg+pini,sizeof(float3)*n,cudaMemcpyDeviceToHost);
  if(gradvelzg)cudaMemcpy(gradvelz,gradvelzg+pini,sizeof(float3)*n,cudaMemcpyDeviceToHost);
  //-Generates VTK file.
  DgSaveVtkParticlesCpu(filename,numfile,0,n,pos,type,idp,vel,rhop,gradrho,gradvelx,gradvely,gradvelz);
  //-Frees memory.
  delete[] pos;
  delete[] check;
  delete[] idp;
